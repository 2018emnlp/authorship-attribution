...
flags.DEFINE_integer("n", 2, "n-grams, 1-gram equals char_dict (default: 2)")
flags.DEFINE_integer("max_len_char", 1000, "Number of characters in a sequence (default: 1000 >> 140)")
flags.DEFINE_integer("max_len_word", 150, "Number of words in a sequence (default: 10)")
flags.DEFINE_integer("num_topics", 150, "Number of LDA topics")

# Eval Parameters
tf.flags.DEFINE_string("checkpoint_dir", "", "Checkpoint directory from training run")
tf.flags.DEFINE_boolean("test_eval", False, "Test eval.py by tiny data")
...
grams_dict = get_json(FLAGS.n_grams_dict + str(FLAGS.n) + ".json")
...
train_char_back = gen_char_batch_back(texts=x_dev, authors=y_dev,
                                      char_dict=grams_dict, author_dict=author_dict,
                                      batch_size=len(y_dev), max_len_char=10, max_len_word=150)
...
with graph.as_default():
    ...
    with sess.as_default():
        ...
        char_back = graph.get_operation_by_name("char_back").outputs[0]
        ...
        x_back, _ = train_char_back.__next__()
        ...
        accuracy = sess.run(accuracy,
                            {char_back: x_back, x_word: x_word_dev, x_topic: x_topic_dev, y: y_dev,
